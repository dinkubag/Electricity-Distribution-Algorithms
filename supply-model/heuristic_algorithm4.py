...
# -*- coding: utf-8 -*-
"""
Created on Wed Mar 12 10:24:55 2025
...
import time
import random
# import electricity_distribution_supplymodel
import copy
...
# =============================================================================
# algorithmm v1 and the required functions    
# =============================================================================

def compute_solution(alg:str, remaining_new_demands: List[Tuple], sorted_std:List[Tuple], rem_supply:float, g:int, k:int) -> ffk.BinsArray :
    '''
    ...
    '''
    logger = logging.getLogger("heuristic_algorithm4.compute_solution")
    ...
    # logging for testing
    # logger.debug(f"Demands to group = {g_items_list},\nThe sum of the demands to group = {g_items_sum}\n")
    ...
    # logging for testing
    # logger.debug(f"packing final remaining demands: {final_remaining_new_demands}, updated supply: {final_rem_supply}\n")
    
    binner = ffk.bkc_ffk()
    if alg == "online":
        solution = ffk.online_ffk(binner, final_rem_supply, final_remaining_new_demands, k)
    else:
        solution = ffk.decreasing_ffk(binner, final_rem_supply, final_remaining_new_demands, k)
    
    numof_bins = binner.numbins(solution)

    # for debugging. It has been borrowed from https://github.com/coin-or/prtpy/blob/main/prtpy/binners.py
    # sums, lists = solution
    # numbins = len(sums)
    ...
    for ibin in range(numof_bins):
        for g_item_idx in range(g):
            binner.add_item_to_bin(bins=solution, item=g_items_list[g_item_idx], bin_index=ibin)
    
    # logger.debug("After adding the items")
    ...
    if alg == "online" or alg == "decreasing":
        for demand in final_remaining_new_demands:
            final_agents_k_vector.append((k, demand[1]))

    # logger.debug(f"Agents vector of k values ={final_agents_k_vector}\n")    
    sums, lists = solution
    
    return (sums, lists, final_agents_k_vector, g_items_list)
    

def compute_egal_alg4_v1(alg:str, demands: List[float], supply: int, k:int=1):
    '''
    We are grouping the demands that can be grouped. For the remaining demand we are applying the kBP algorithm.
    Among the different groups possible we are returning the one whose leximin allocation of supply is highest.

    NOTE: we do not require to compute the set of very small demands, because we are using kBP, and 'k' is same for all
    the demands.
    ...
    for demand in new_demands:
        if demand[0] <= largest_demand[0] and sum_smallest_tolargest_demand + demand[0] <= supply - max_demand_agent[0] :
    ...
    # logging
    # logger.debug(f"Demands (sorted) that can be grouped= {sorted_smallest_tolargest_demand}")
    # logger.debug(f"The sum of demands that can be grouped={sum_smallest_tolargest_demand}\n")
    
    # number of items in sorted_smallest_tolargest_demand
    numof_items_instd = len(smallest_tolargest_demand)
    # logger.debug(f"There are {numof_items_instd} items that can be grouped.\n")
    ...
    if sum([item[0] for item in new_demands]) <= supply:
        ...
        final_rem_supply = supply
        
        # a better version of below line
        each_agents_k_vector = [(1, item[1]) for item in new_demands]
        
        # logger.info("Sum of demands <= supply, so all agents are connected.")
        # logger.debug(f"Agents vector of k values ={each_agents_k_vector}\n")    
        
        binner = ffk.bkc_ffk()
        if alg == "online":
            solution = ffk.online_ffk(binner, final_rem_supply, new_demands, 1)
        else:
            solution = ffk.decreasing_ffk(binner, final_rem_supply, new_demands, 1)
        sums, lists = solution
                
        # Now we will add this solution to the list of different solutions.
        soln  = Solution(sums, lists, new_demands, each_agents_k_vector, group_size = 0, k=k, group_list=[])
        ...
        # logger.info("\n\n****FINAL SOLUTION****")
        # sums = best_solution.sums
        # lists = best_solution.lists
        # numbins = len(sums)
        # bin_str = [f"Bin #{i}: {lists[i]}, sum={sums[i]}" for i in range(numbins)]
        # logger.debug(f"{bin_str}\n")
        ...
        # logger.info("All agents are connected all the time, so no need to compute max and min allocated supply.")
        # logger.info("\n\n\n\n")
        
        # logger.debug(f"{alg}, {1}, {best_solution.min_asr[0]}")
        # logging ---------- END
        
        # uncomment the below line if using the main algorithm on the electricity data.
    ...
    while end - begin > 3:
        ...
        soln_begin = compute_solution(alg, new_demands, sorted_smallest_tolargest_demand, supply, begin, k)
        # compute the solution for g = end
        soln_end = compute_solution(alg, new_demands, sorted_smallest_tolargest_demand, supply, end, k)
        
        sums_begin, lists_begin, agents_k_vector_begin, g_items_list_begin = soln_begin
        sums_end, lists_end, agents_k_vector_end, g_items_list_end = soln_end
        
        # Now we will add this solution to the list of different solutions.
        solution_begin  = Solution(sums_begin, lists_begin, new_demands, agents_k_vector_begin, group_size = begin, g=begin, group_list=g_items_list_begin)
        solution_end  = Solution(sums_end, lists_end, new_demands, agents_k_vector_end, group_size = end, g=end, group_list=g_items_list_end)
    ...
    for grp in range(begin, end+1):
        ...
        sums, lists, agents_k_vector, g_items_list = soln_grp
        
        solution_grp = Solution(sums, lists, new_demands, agents_k_vector, group_size = grp , g=grp, group_list=g_items_list)
    ...
    # comment the below one line when running on main algorithm
    return best_solution
    # uncomment if testing the function.
    # return
